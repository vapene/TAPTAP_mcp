...
parser.add_argument('--subjects', type=str, default='한국지리', help='Comma-separated list of subjects to include') # 수학Ⅰ,수학Ⅱ
...
parser.add_argument('--trial', type=int, default=10, help='numer of trials')
...
args = load_best_configs(args, "configs_NA2.yml")
...
try:
    train_data = pd.read_csv(f"./processed/{args.num_problems}_{args.subjects}_30_ratio_afterNA2.csv")  # Assume file path is correct
...
columns_to_keep = ['user_id', 'item_id', 'score','original']
train_data = train_data[columns_to_keep]

train_data.dropna(inplace=True)
train_data['score'] = train_data['score'].astype(int) ###float에서 int로 바꿈(성능 변화 확인)


# Separate original and non-original data
original_data = train_data[train_data['original'] == 'Y'].drop(columns=['original'])
non_original_data = train_data[train_data['original'] != 'Y'].drop(columns=['original'])
# Total number of samples
total_samples = len(train_data)
# Calculate the number of samples for each split
num_test_samples = int(total_samples * 0.15)
num_valid_samples = int(total_samples * 0.15)
num_train_samples = total_samples - num_test_samples - num_valid_samples
...
for i in range(10):
    try:
        test = original_data.sample(n=num_test_samples, random_state=i)
        remaining_original_data = original_data.drop(test.index)
        combined_data_for_valid = pd.concat([remaining_original_data, non_original_data])
        valid = combined_data_for_valid.sample(n=num_valid_samples, random_state=i)
        train = combined_data_for_valid.drop(valid.index)
