...
# subjects=("독해" "독서,문학" "물리학Ⅰ" "화학Ⅰ" "생명과학Ⅰ" "지구과학Ⅰ" "한국사" "생활과 윤리" "정치와 법" "한국지리" "세계지리" "사회?문화" "동아시아사" "세계사" "윤리와 사상")
# subjects=("$korean_combined")
...
for (( cycle = 0; cycle < total_cycles; cycle++ )); do
    for subject in "${subjects[@]}"; do
        ...
        for (( i = 0; i < iterations_per_cycle; i++ )); do
            ...
            output=$(python IRT_mine_NA2.py --trial $trial --timestamp "$timestamp" --subjects "$subject" --alpha $alpha --gamma $gamma --batch_size $batch_size --epochs $epochs --patience $patience --seed $seed --param_type $param_type --train_ratio $train_ratio)
            ...
            if [[ $(echo "$current_score > ${best_scores[$subject]}" | bc -l) -eq 1 ]]; then
                ...
                echo -e "$best_params" > "results_NA2/$subject/best_params_${timestamp}.txt"
