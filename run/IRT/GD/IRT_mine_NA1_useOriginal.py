...
parser.add_argument('--subjects', type=str, default='한국지리', help='Comma-separated list of subjects to include') # 물리학Ⅰ
...
print('len train',len(train_data['score']))
...
print('non',non_original_data.shape)
...
for i in range(10):
    try:
        ...
        remaining_original_data = original_data.drop(test.index).reset_index(drop=True)
        combined_data_for_valid = pd.concat([remaining_original_data, non_original_data]).reset_index(drop=True)
        valid = combined_data_for_valid.sample(n=num_valid_samples, random_state=i)
        train = combined_data_for_valid.drop(valid.index).reset_index(drop=True)
