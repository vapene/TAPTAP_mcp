...
class IRT_freeze_abc(CDM):
    ...
    def load_leaveoneout(self, weights_dict, num_users, num_items, leaveoneout_index=10):
        with torch.no_grad():  
            ...
            indices_to_update = all_indices[:leaveoneout_index-1] + all_indices[leaveoneout_index:]
            mask = torch.tensor(indices_to_update)
            print("weights_dict['theta.weight']",weights_dict['theta.weight'][:10])
            self.irt_net.theta.weight.data[:-1] = weights_dict['theta.weight'].clone().detach()[mask]
            print('self.irt_net_theta_weight',self.irt_net.theta.weight.data[:10])
